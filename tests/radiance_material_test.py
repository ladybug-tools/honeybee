"""A module for testing all Radiance materials. Add more materials as the library expands."""
...
class MaterialTypeTestCase(unittest.TestCase):
    ...
    def test_material_definitions(self):
        #The idea is to test material definitions through toRadString
        #I'm using string strip to avoid any whitespace related false-errors.

        #glass1
        self.assertEqual(tuple(self.glass1.toRadString().split()),
                         ('void','glass','glazing','0','0','4','0.545','0.545','0.545',
                          '1.520'))
        #glow1
        self.assertEqual(tuple(self.glow1.toRadString().split()),
                         ('void','glow','glow1','0','0','4','200.000','1000000.000',
                          '200.000','5.000'))

        #bsdf1
        self.assertEqual(tuple(self.bsdf1.toRadString().split()),
                         ('void','BSDF','clear','6','0.000','assets/clear.xml',
                          '0.010','0.010','1.000','.','0','0'))

        #light1
        self.assertEqual(tuple(self.light1.toRadString().split()),
                         ('void','light','light1','0','0','3','1000000.000','0.000','0.000'))

        #metal1
        self.assertEqual(tuple(self.metal1.toRadString().split()),
                         ('void','metal','aluminium','0','0','5','0.500','0.500','0.500',
                          '0.100','0.001'))

        #plastic1
        self.assertEqual(tuple(self.plastic1.toRadString().split()),

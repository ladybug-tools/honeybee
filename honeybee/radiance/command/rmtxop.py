...
from ..datatype import RadiancePath, RadianceTuple, RadianceValue,RadianceBoolFlag
...
class Rmtxop(RadianceCommand):
    ...
    #Advanced usage with transformations and such. In this case I am subtracting one matrix and adding another.
    finalMatrix = Rmtxop()
            ...
            if isinstance(rmtxopMatrices,RmtxopMatrix):
                ...
                try:
                    ...
                    for idx,matrix in enumerate(rmtxopMatrices):
                        assert isinstance(matrix,RmtxopMatrix),'The input #%s is not an RmtxopMatrix'%idx
                        self.__rmtxopMatrices.append(matrix)
                except:
                    raise Exception("The input for rmtxopMatrices should either be a single instance of the class"
                                    " RmtxopMatrix or a list/tuple/iterable containing multiple instances of the"
                                    "RmtxopMatrix")
        else:
    ...
    def toRadString(self, relativePath=False):
        ...
        #If matrices are complex .i.e containing transformations scalars etc., get to them first.
        compoundMatrices = ''
        ...
        if self.matrixFiles:
            ...
            #If compound matrices have already been specified, then add a plus in the beginning.
            if compoundMatrices:

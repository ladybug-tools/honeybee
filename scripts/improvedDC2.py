...
def improvedDCcalc(epwFile,materialFile, geometryFiles, pointsFile,folderForCalculations,
                   outputIllFilePath, glazingGeometry=None,
                   reinhartPatches=1, ambBounces=5,ambDiv=5000,
                   limitWt= 0.0002, calcDict={'skyVectors':True,'DirectDCCoeff':True,'DCCoeff':True,'DirectSun':True},
    ...
    weaFilePath = os.path.join(tmpFolder,os.path.splitext(os.path.split(epwFile)[1])[0]+'.wea')
    ...
    if calcDict['DirectSun']:
        sunOnlyIll = calcDirectIlluminance(epwFile=epwFile, solarDiscPath=solarDiscPath,
...
if __name__ == "__main__":
    ...
    epw = os.path.join(os.getcwd(),'tests/room','test.epw')
    sunRad = 'tests/assets/phoenixSunPath.rad'

    sunList = 'tests/assets/phoenixList.txt'
    sunMtx = 'tests/assets/phooenixMtx.mtx'

    materialFile = r'tests/room/room.mat'
    geometryFiles = os.path.abspath('tests/room/room.rad')
    glazingGeometry = r'tests/room/glazing.rad'
    pointsFile = os.path.abspath(r'tests/room/indoor_points.pts')

    calcFolder = os.path.abspath(r'tests/room')
    outputFilePath = r'tests/room/test.ill'
    ...
    improvedDCcalc(epwFile=epw,  materialFile=materialFile, geometryFiles=geometryFiles, pointsFile=pointsFile,
                   ...
                   calcDict={'skyVectors': True, 'DirectDCCoeff': True, 'DCCoeff': True, 'DirectSun': True})

...
def analemma(epwFile, sunDiscRadPath, sunListPath=None, solarRadiationMatrixPath=None, HOYlist=range(8760)):
    """
    This function can be used for multiple purposes. For just generating the sunDiscRadPath for visualizing
    physically-accurate sun positions, specify only that variable. For calculating ASE or for Daylight Coefficient
    calculations, specify the sunListPath and solarRadiationMatrixPath as well.
    Args:
        epwFile: A standard epw file containing 8760 data points. Hacked epws will also work but then the HOY input
            needs to be adjusted accordingly. Required input.
        sunDiscRadPath: The path for the rad file to which the Radiance definitions of solar discs will be written to.
        sunListPath: The path for the list of suns which is required for daylighting calculations.
    ...
    if sunListPath or solarRadiationMatrixPath:
        assert sunListPath and solarRadiationMatrixPath,\
            'If either sunListPath (%s) or solarRadiationMatrixPath (%s) are specified, the other variable needs to be specified' \
